...
/**
 * Get the database path for RAG vector store
 */
function getRagVectorDbPath(): string {
...
const DB_PATH = getRagVectorDbPath();
...
/**
 * Vector Store for Ortofaccia knowledge base
 * Uses LibSQL Vector for local vector storage
 */
export class OrtofacciaVectorStore {
  private store: LibSQLVector;
  private indexName = 'ortofaccia_knowledge';

  constructor() {
    // Use file-based storage for persistence
    // Using absolute path to ensure consistency between seed script and runtime
  ...
  async initialize(): Promise<void> {
    try {
      console.log('Initializing Ortofaccia vector store...');
  ...
  async upsertChunks(chunks: EmbeddedChunk[]): Promise<void> {
    try {
      ...
      const ids = chunks.map(
        chunk => `${chunk.metadata.filename}-chunk-${chunk.metadata.chunkIndex}`
      );
   ...
   */
  async getStats(): Promise<{ indexName: string; message: string }> {
    return {
      indexName: this.indexName,
...
export const ortofacciaVectorStore = new OrtofacciaVectorStore();
